...
public class GhostBlockCommand extends CommandBase {
        ...
        if ("set".equals(subCommand)) {
            ...
            for (String arg : optionalArgs) {
                if (arg.equalsIgnoreCase("-s") || arg.equalsIgnoreCase("--save")) {
                    ...
                    throw new WrongUsageException(getCommandUsage(sender)); // 无效参数
                }
            }
            ...
            notifyOperators(sender, this, "ghostblock.commands.cghostblock.set.success", 
            ...
            if (useBatch) {
                ...
                notifyOperators(sender, this, "ghostblock.commands.cghostblock.fill.batch_started", allBlocks.size(), batchSize);
            } else {
                int count = fillGhostBlocks(world, from, to, state);
                if (saveToFile) {
                    saveGhostBlocks(world, allBlocks, state);
                }
                notifyOperators(sender, this, "ghostblock.commands.cghostblock.fill.success", count);
            }
            ...
            if (hasFlag(args, "-b", "--batch")) { // 使用批量加载
                ...
                notifyOperators(sender, this, "ghostblock.commands.cghostblock.load.batch_started", 
                ...
                for (GhostBlockData.GhostBlockEntry entry : entries) {
                    ...
                    if (block != null) {
                        world.setBlockState(pos, block.getStateFromMeta(entry.metadata), 0x10 | 0x40);
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 0) {
            return getListOfStringsMatchingLastWord(args, "set", "fill", "load");
    ...
    private void setGhostBlock(WorldClient world, BlockPos pos, BlockStateProxy state) throws CommandException {
        ...
        if (world.isRemote) {
            ...
            if (block != null) {
                // 设置方块状态并强制渲染
                world.setBlockState(pos, block.getStateFromMeta(state.metadata), 0x10 | 0x40);
                world.markBlockRangeForRenderUpdate(pos, pos);
