...
public class GhostBlockCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        if ("set".equals(subCommand)) {
            if (args.length < 5) throw new WrongUsageException(getCommandUsage(sender));
            ...
            if (args.length < 8) throw new WrongUsageException(getCommandUsage(sender));
            ...
            if (useBatch) {
                ...
                sender.addChatMessage(formatMessage("ghostblock.commands.cghostblock.fill.batch_started", 
            ...
            if (entries.isEmpty()) {
                sender.addChatMessage(formatMessage(EnumChatFormatting.YELLOW, "ghostblock.commands.load.empty"));
            } else {
                sender.addChatMessage(formatMessage(EnumChatFormatting.GREEN, 
            ...
            if (hasFlag(args, "-b", "--batch")) {
                ...
                sender.addChatMessage(formatMessage("ghostblock.commands.load.batch_started", 
                ...
                for (GhostBlockData.GhostBlockEntry entry : entries) {
                    ...
                    if (block != null) {
                        world.setBlockState(pos, block.getStateFromMeta(entry.metadata), 3);
    ...
    private void handleClearCommand(ICommandSender sender, World world, String[] args) throws CommandException {
        ...
        if (args.length >= 3 && "confirm".equalsIgnoreCase(args[2])) {
            ...
            if (confirmation.targetFile.delete()) {
                pendingConfirmations.remove(senderKey);
                sender.addChatMessage(formatMessage(EnumChatFormatting.GREEN, "ghostblock.commands.clear.success"));
            ...
            message.appendSibling(new ChatComponentText("/cghostblock clear file confirm")
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            return getListOfStringsMatchingLastWord(args, "set", "fill", "load", "clear");
        } else if (args.length == 2 && "clear".equalsIgnoreCase(args[0])) {
            return getListOfStringsMatchingLastWord(args, "file");
    ...
    private List<String> handleSetTabCompletion(ICommandSender sender, String[] args) {
        ...
        if (currentArgIndex >= 1 && currentArgIndex <= 3) {
            ...
            return getListOfStringsMatchingLastWord(args, suggestions.toArray(new String[0]));
    ...
    private List<String> handleLoadTabCompletion(String[] args) {
        ...
        if (currentArgIndex == 1) {
            ...
            if (flagIndex != -1 && currentArgIndex == flagIndex + 1) {
                return getListOfStringsMatchingLastWord(args, "100", "500", "1000");
            }
        }
        return Collections.emptyList();
    }
    ...
    private BlockPos parseBlockPos(ICommandSender sender, String[] args, int index) throws CommandException {
        ...
        double baseX = (player != null) ? player.posX : 0;
        double baseY = (player != null) ? player.posY : 0;
        double baseZ = (player != null) ? player.posZ : 0;
    ...
    private BlockStateProxy parseBlockState(String input) throws CommandException {
        try {
            ...
            System.out.println("[SUCCESS] 解析结果: " + block.getRegistryName() + " (ID=" + Block.getIdFromBlock(block) + "), meta=" + meta);
    ...
    private void setGhostBlock(WorldClient world, BlockPos pos, BlockStateProxy state) throws CommandException {
        ...
        if (world.isRemote) {
            ...
            if (block != null) {
                ...
                System.out.println("[SUCCESS] 已设置幽灵方块: " + block.getLocalizedName() + " at " + pos);
                if (block == Blocks.air) {
    ...
    public static void onWorldLoad(WorldEvent.Load event) {
        System.out.println("[GhostBlock-DEBUG] 世界加载事件触发，是否为客户端世界？ " + event.world.isRemote);
